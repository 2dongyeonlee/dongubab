...
- **① 이 프로젝트에서만** — `.mcp.json` (Claude Code가 프로젝트 루트에서 자동으로 읽음)
- **② 모든 프로젝트에서 (전역)** — `install-mcp.sh` 를 내 컴퓨터에서 한 번 실행

### 🌍 모든 프로젝트에서 쓰기 (전역 설치)

내 컴퓨터에서 아래를 한 번만 실행하면 유저 스코프로 깔려서, 이후 **어느 폴더에서
Claude Code를 켜도** 이 서버들이 뜹니다.

```bash
bash install-mcp.sh      # 키 있으면 먼저 export 하고 실행 (아래 참고)
claude mcp list          # 설치 확인
```

> 원격(웹) 세션에서는 유저 스코프 파일이 네 로컬 컴퓨터에 없기 때문에, 전역 설치는
> **반드시 네 컴퓨터의 터미널에서** 실행해야 합니다.
> `filesystem`은 전역이라 대상 폴더를 홈(`$HOME`)으로 잡아뒀습니다 — 범위를 좁히려면
> 스크립트의 `"$HOME"` 을 원하는 폴더로 바꾸세요.
